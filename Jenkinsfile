pipeline {
    ...
    environment {
        versionFile = 'thetiptop_front_version'
            ...
            steps {
                ...
                withSonarQubeEnv('SonarQube#1') {
                    sh """
                        ...
                        -Dsonar.projectKey=thetiptop_front \
                        -Dsonar.sources=src \
                        -X
                    """
                }
        ...
        stage('Build') {
            ...
            steps {
                script {
                    ...
                    docker.build("dsp5archi/thetiptop_frontend:$version", "-f Dockerfile.dev .")
                }
            }
        }
            ...
            steps {
                script {
                    docker.withRegistry('https://index.docker.io/v1/', 'dockerhub_credentials') {
                        docker.image("dsp5archi/thetiptop_frontend:$version").push("$version")
                        docker.image("dsp5archi/thetiptop_frontend:$version").push("latest")
                    }
                }
            }
        }
            ...
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'gitea', usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD')]) {
                        ...
                        '''
                            def remoteUrl = 'https://gitea.dsp5-archi-f23-15m-g4.fr/dsp/thetiptop_front.git'
